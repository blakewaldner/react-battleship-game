...
import { BattleshipMove, TownsService, TownsServiceClient } from '../generated/client';
...
import BattleshipArea from '../components/Town/interactables/BattleshipArea';
import BattleshipAreaController from './BattleshipAreaController';
...
export type TownEvents = {
  ...
  battleshipAreasChanged: (newBattleshipAreas: BattleshipAreaController[]) => void;
...
export default class TownController extends (EventEmitter as new () => TypedEmitter<TownEvents>) {
  ...
  private _battleshipAreas: BattleshipAreaController[] = [];
  ...
  public get battleshipAreas() {
    return this._battleshipAreas;
  }

  public set battleshipAreas(newbattleshipAreas: BattleshipAreaController[]) {
    this._battleshipAreas = newbattleshipAreas;
    this.emit('battleshipAreasChanged', newbattleshipAreas);
  }
  ...
  registerSocketListeners() {
    ...
    /**
     * Listening from townService
     * REALLY SUS CODE >:(
     */
    this._socket.on('battleshipGame', () => {
      this.emit('battleshipAreasChanged', this._battleshipAreas);
    });
    ...
    this._socket.on('interactableUpdate', interactable => {
      console.log('TOWN CONTROLLER FRONT END IF STATEMENT');
      ...
      } else if (isBattleshipArea(interactable)) {
        const relArea = this.battleshipAreas.find(area => area.id == interactable.id);
        if (relArea) {
          console.log('TOWN CONTROLLER relAREA', relArea);
          console.log('TOWN CONTROLLER INTERACTABLE', interactable);
          relArea.updateFrom(interactable);
          console.log('AFTER UPDATE FROM');
        }
      }
    });
  }
  ...
  async createBattleshipArea(newArea: BattleshipAreaModel) {
    // TODO catch the error if the file type is invalid
    console.warn('Intermediate batttleship: ' + JSON.stringify(newArea, null, 4));
    await this._townsService.createBattleshipArea(this.townID, this.sessionToken, newArea);
  }
      ...
      this._socket.on('initialize', initialData => {
        ...
        initialData.interactables.forEach(eachInteractable => {
          if (isConversationArea(eachInteractable)) {
            ...
            this._battleshipAreas.push(new BattleshipAreaController(eachInteractable));
          }
        });
  ...
  /**
   * Retrieve the battle ship area controller that corresponds to a battleshipAreaModel, creating one if necessary
   *
   * @param battleshipArea
   * @returns
   */
  public getBattleshipAreaController(battleshipArea: BattleshipArea): BattleshipAreaController {
    const existingController = this._battleshipAreas.find(
      eachExistingArea => eachExistingArea.id === battleshipArea.name,
    );
    if (existingController) {
      return existingController;
    } else {
      const newController = new BattleshipAreaController({
        id: battleshipArea.name,
        turn: '',
        gameState: 'setup',
        battleshipGuessedBoards: {},
        battleshipSetups: {}, //TODO
      });
      this._battleshipAreas.push(newController);
      return newController;
    }
  }
  ...
  public emitBattleshipAreaUpdate(battleshipArea: BattleshipAreaController) {
    this._socket.emit('interactableUpdate', battleshipArea.battleshipAreaModel());
  }
  ...
  public async joinBattleshipGame(
    battleshipArea: BattleshipAreaController,
    gameboard: string[][],
  ): Promise<GameInfo> {
    return this._townsService.joinGame(
      this.townID,
      battleshipArea.id,
      this.sessionToken,
      this.ourPlayer.id || 'invalid player id',
      gameboard,
    );
  }
  ...
  public async submitMove(
    ...
    return this._townsService.submitMove(
      this.townID,
      battleshipArea.id,
      this.sessionToken,
      this.ourPlayer.id || 'invalid player id',
...
export function useBattleshipAreaController(battleshipAreaID: string): BattleshipAreaController {
  const townController = useTownController();
  const ret = townController.battleshipAreas.find(eachArea => eachArea.id === battleshipAreaID);
  if (!ret) {
    throw new Error(`Unable to locate battleship area ${battleshipAreaID}`);
  }
  return ret;
}
