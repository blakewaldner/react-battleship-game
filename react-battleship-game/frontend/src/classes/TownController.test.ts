...
describe('TownController', () => {
  ...
  describe('With a successful connection', () => {
    ...
    describe('[T2] interactableUpdate events', () => {
      ...
      describe('BattleshipAreaController updates', () => {
        function battleshipAreaOnTown() {
          return {
            ...(townJoinResponse.interactables.find(eachInteractable =>
              isBattleshipArea(eachInteractable),
            ) as BattleshipArea),
          };
        }
        let battleshipArea: BattleshipArea;
        let battleshipAreaController: BattleshipAreaController;
        let eventListener: (update: BattleshipArea) => void;
        beforeEach(() => {
          battleshipArea = battleshipAreaOnTown();
          const controller = testController.battleshipAreas.find(
            eachArea => eachArea.id === battleshipArea.id,
          );
          if (!controller) {
            fail(
              `Could not find battleship area controller for battleship area ${battleshipArea.id}`,
            );
          }
          battleshipAreaController = controller;
          eventListener = getEventListener(mockSocket, 'interactableUpdate');
        });
        it('interactableUpdate and battleship hooks Updates the battleship area model', () => {
          battleshipArea.gameState = 'over';
          battleshipArea.turn = nanoid();
          battleshipArea.battleshipSetups = { test: [['']] };
          battleshipArea.battleshipGuessedBoards = {};

          eventListener(battleshipArea);

          expect(battleshipAreaController.battleshipAreaModel()).toEqual(battleshipArea);
        });
        it('interactableUpdate and battleship hooks Emits a gameStateChange event if the game state changes', () => {
          const listener = jest.fn();
          battleshipAreaController.addListener('gameStateChange', listener);
          battleshipArea.gameState = 'over';
          eventListener(battleshipArea);
          expect(listener).toBeCalledWith(battleshipArea.gameState);
        });
        it('interactableUpdate and battleship hooks Emits a battleshipSetupsChange event if the battleship setups change', () => {
          const listener = jest.fn();
          battleshipAreaController.addListener('battleshipSetupsChange', listener);
          battleshipArea.battleshipSetups = { test: [['']] };
          eventListener(battleshipArea);
          expect(listener).toBeCalledWith(battleshipArea.battleshipSetups);
        });
        it('interactableUpdate and battleship hooks Emits a battleshipGuessedBoardsChange event if the battleship guessed boards change', () => {
          const listener = jest.fn();
          battleshipAreaController.addListener('battleshipGuessedBoardsChange', listener);
          battleshipArea.battleshipGuessedBoards = { test: { board: [[]], shipsSunk: 0 } };
          eventListener(battleshipArea);
          expect(listener).toBeCalledWith(battleshipArea.battleshipGuessedBoards);
        });
        it('interactableUpdate and battleship hooks Emits a turnChange event if the turn changes', () => {
          const listener = jest.fn();
          battleshipAreaController.addListener('turnChange', listener);

          battleshipArea.turn = nanoid();
          eventListener(battleshipArea);
          expect(listener).toBeCalledWith(battleshipArea.turn);
        });
      });
    });
  });
