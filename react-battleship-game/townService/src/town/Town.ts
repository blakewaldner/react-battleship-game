...
import { isBattleshipArea, isPosterSessionArea, isViewingArea } from '../TestUtils';
import {
  ChatMessage,
...
export default class Town {
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    socket.on('interactableUpdate', (update: Interactable) => {
      ...
      } else if (isBattleshipArea(update)) {
        // forward interactableUpdate
        newPlayer.townEmitter.emit('interactableUpdate', update);
        const existingBattleshipArea = <BattleshipArea>(
          this._interactables.find(area => area.id === update.id && area instanceof BattleshipArea)
        );
        if (existingBattleshipArea) {
          existingBattleshipArea.updateModel(update);
        }
      }
    });
    return newPlayer;
  ...
  public initializeFromMap(map: ITiledMap) {
    ...
    const battleshipAreas = objectLayer.objects
      .filter(eachObject => eachObject.type === 'BattleshipArea')
      .map(eachPSAreaObj => BattleshipArea.fromMapObject(eachPSAreaObj, this._broadcastEmitter));

    this._interactables = this._interactables
      .concat(viewingAreas)
      .concat(conversationAreas)
      .concat(posterSessionAreas)
      .concat(battleshipAreas);
